# 大模型的核心——Transformer 架构自注意力机制

## 1 Transformer 架构
![](./images/image.png)
其中，左侧为 Encoder Block, 右侧为 Decoder Block
...
本质：一个 Element-wise 算子。如果是 RoPE, 则用来实现实数域到复数域旋转的坐标变换。

作用：在输入/输出 Embedding 向量上加一个“位置信号”，让模型知道这个词的具体位置。让模型具备处理序列顺序的能力。

业界常用的位置编码算子：RoPE

###### RoPE (Rotary Positional Encoding)
作用：利用复数旋转，将 token 的位置点旋转一个特定的角度。

`f(x,m) = R*W*x`

特性：
1. 相对性：两个 token 之间的 Attention 分数，只取决于相对距离 (m - n), 而非绝对位置
2. 外推性：旋转是连续的，处理长文本时比传统方式（固定编码）更稳定
3. 计算友好：底层实现，只需通过矩阵向量乘、三角函数查表快速实现，不需要额外的 Embedding 层

#### 1.1.2 编码器(Encoder)
核心：前馈神经网络、多头注意力

##### 多头注意力(MHA)
作用：**上下文建模。** 让每个 token “观察”句子中的所有其他 token, 根据相关性权重聚合信息。同时消除歧义，让向量往相关性更高的一侧偏移。

步骤：
1. Linear Projection: 输入映射为 Q, K ,V
2. Dot-Product: 矩阵乘 QK(T), 计算相似度分数
3. Softmax: 归一化权重
4. Weighted Sum: 用权重对 V 加权求和

##### 前馈神经网络(Feed Forward Network)
作用：**特征加工与记忆库**。 将注意力机制收集到的信息，进行“消化”和映射。

构成：两层全连接层，中间夹一个激活函数 (ReLU/GELU/SwiGLU)

#### 1.1.3 解码器(Decoder)
核心：掩码多头注意力、多头注意力、前馈神经网络

##### 掩码多头注意力(Masked MHA)
作用：将未来位置的注意力分数设为负无穷，切换反向信息流，保证模型只根据已生成的词来预测下一个词。

##### 交叉注意力(Cross-Attenion): 编码器-解码器架构独有
作用：连接输入和输出。适用于模型翻译和生成摘要，每一刻都能检查原文中哪些词相关性较高。

#### 1.1.4 输出层
核心：Linear + Softmax

作用：从高维特征空间“坍缩”到一个具体的词。Linear 负责将隐藏层的向量映射到一个超大维度的空间（匹配词表大小）。Softmax 则将原始分值转化为 (0,1) 间的概率分布
...
原理：在特征维度计算均值和方差，把数据标准化为正态分布，再加上缩放和偏移两个参数。

##### 残差连接(Residual Connection / Add)
作用：保护梯度流，防止模型随层数增加而退化。


### 1.2 三大架构
1. Decoder-only: 单向可见，采用因果掩码。是主流 LLM 的标配；擅长文本生成、推理。如 GPT、 Llama 模型
2. Encoder-only: 双向可见。适用于文本立即、分类、命名实体识别。如 BERT 模型
3. Encoder-Decoder: 混合式，Encoder 处理输入（全可见），Decoder 处理输出（因果可见），中间通过交叉注意力链接。适用于机器翻译、摘要提取。如 T5、BART 模型

#### Decoder-only (主流)
核心：使用掩码的自注意力机制，过去已计算过的 Token, 其表征不再受未来 Token 的影响。即单向信息流。

原理：在生成第 i 个 Token 时，在计算 \[0, i) 内 Token 时生成的 K(key) 和 V(value) 是不变的。这时可以利用 **KV Cache** （KV 缓存）将其保存下来，后续 Token 的生成可以使用，再单独计算第 i 个 Token 的 Q、K、V 并与之前的 KV Cache 做运算即可，极大降低了内存压力。

Encoder (双向) 的缺点：第 i 个 Token 生成时，也会影响 \[0, i) 内所有 Token 的注意力分布，需要全部跑一遍模型。

##### 为什么 Decoder-only 成为了主流？
1. **KV Cache 极度友好**：能够缓存 K、V 的中间计算结果，把推理复杂度从 O(N^2) 降至 O(N)，极大地减轻了 GPU 内存的数据存储和传输压力。
2. 自回归任务的适配度：序列化推理计算，即根据“从已知推到未知”，天然地适合 NLP 及文本处理。
3. 工程扩展性：堆叠方式单一，适合并行化扩展参数的规模。

### 1.3 自注意力 (Self-Attention) 机制剖析
核心组件：Q, K, V

#### 1.3.1 预处理： Q, K, V 矩阵的计算
1. Query(Q): 当前 Token 要寻找的上下文信息
2. Key(K): 当前 Token 能提供给其他 Token 的特征标签
3. Value(V): 匹配成功时，真正要传递的信息

#### 1.3.2 注意力的计算步骤
1. 计算 Q, K, V: 输入向量 xi 分别乘以 Q, K, V 的权重矩阵 WQ, WK, WV
2. 计算注意力分数（点乘）：用当前词的 q 向量 点乘句子中的所有词的 k 向量。注意力分数 score 的意义，决定了在处理当前词时，要投入多少注意力在其他词上。
3. 缩放：将分数除以 sqrt(dk), 其中 dk 代表 Key 向量的维度。目的是使**梯度更稳定**，防止点成结果过大，导致下一步 Softmax 进入梯度极小的区域（饱和区），导致梯度消失。
4. 归一化 (Softmax): 将所有分数变为正数且总和为 1，即按概率分布。
5. 加权求和：将每个词的 Softmax 分数，乘以对应的 Value 向量 v, 然后求和, 得到向量 z1。 z1 代表当前词 (如 "Thinking") 融合了上下文信息后的新特征。

##### 缩放的作用：为什么要除以 sqrt(dk) ?
核心原因：避免**方差爆炸**， Softmax 计算结果饱和。

Softmax 饱和是致命的：

##### Online Softmax: 低精度 (FP16) 时的通用解

## 2 分布式计算
当模型规模极大时，如何通过多卡甚至多机，分布式处理模型计算？

### 2.1 分布式并行策略
#### 2.1.1 数据并行 (Data Parallel)

#### 2.1.2 模型并行

#### 2.1.2.1 张量并行 (Tensor Parallel)

#### 2.1.2.2 流水线并行 (Pipeline Parallel)

#### 2.1.2 异构并行
即使用 GPU 内存 + CPU 内存 + NVMe 磁盘，后两者存放不经常复用的数据

### 2.2 ZeRO 技术 —— 优化器的并行
模型规模庞大，导致单个 GPU 显存难以承载全部的模型。

核心：将训练过程中最占显存的部分（优化器状态、模型参数、梯度），去除冗余。

关键技术：**ZeRO (零冗余优化器)**。通过通信 (All-Gather) 换区显存。只在计算时传输和使用需要的参数，用完即删除，消除内存数据冗余。

ZeRO 的三种切分：
- ZeRO-1: 优化器状态切分
- ZeRO-2: 优化器状态和梯度切分
- ZeRO-3: 优化器状态、梯度和模型权重参数切分

### 2.3 多维混合并行 —— 多种并行技术的结合
结合数据并行 + 模型并行 + 流水线并行的技术，用于一机多卡、多机多卡的分布式训练。

#### 多机多卡的混合并行
核心：重复利用网络带宽。张量并行所需通信量最大。

策略：一机内 (NVLink, 带宽高) 张量并行，多机之间 (网络带宽，较慢) 使用数据+流水线并行

## 3 显存优化技术
### 3.1 KV Cache 优化
痛点：在有限的 GPU 的 SRAM (片内共享内存) & HBM (全局高速内存) 之间，解决“内存碎片化”和“带宽瓶颈”的问题。

#### 显存分页管理: PagedAttention
核心模型：vLLM

vLLM之前传统模型的痛点：连续内存分配
- 方式：预分配 KV Cache 空间。按最大序列长度（如 2048）预分配一段连续的内存。
- 实际：每次请求生成的 Token 长度中，只有一小部分的空间用于存储实际的 token 状态，剩下的框架被浪费。造成**内存碎片化**。
- 痛点：显存空间被大量浪费，严重限制了 Batch Size 和吞吐量。

PagedAttention 的核心原理：借鉴 OS 虚拟内存分页。KV Cache 在逻辑上连续，而在物理显存上可不连续，避免了显存碎片化的问题。

逻辑 KV 内存块（连续） -> 块表 -> 物理 KV 内存块（不一定连续）

#### 算子融合与带宽优化: FlashAttention-2
核心：通过算子融合与 IO 感知优化，解决“带宽瓶颈”问题。

常规的 Attention 计算，需经理**八次内存读写**。
1. 从显存读取 Q, K, 计算 S = QK(T)， 写 S 到显存
2. 读 S, 计算 P = Softmax(S), 写 P 到显存
3. 读 P, V, 计算 O = PV, 写 O 到显存

痛点：
1. 内存读写开销极大：中间矩阵 S, P 都是 N x N 维度，读写速度极慢，远跟不上计算速度。
2. 共享内存 (SRAM) 浪费

改进 —— FlashAttention 的核心技术：**内存分块 (Tiling)**。

##### A. 分块计算 (Tiling)
1. 分块：将 Q, K, V 分块，一般为 128 x 128 或 256 x 256 (与 GPU 共享内存适配)。
2. SRAM 迁移：在 CUDA 核函数中，将一小块 Q, K 写入 SRAM
3. SRAM 内部计算：直接在共享内存中计算 Softmax, 结果与对应 V 分块相乘，得到输出 O 的部分和

优化：舍弃了中间计算结果 S, P, 无需单独写入内存，极大地节省了显存空间、减少读写开销；并充分利用了 高带宽的 SRAM 

##### B. 增量 Softmax (Online Softmax)
核心：局部最大值的“重新缩放”。没处理一组新的分块时，都根据当前分块计算出的局部最大值，将已经计算好的部分结果进行重新缩放。

##### C. 反向传播的重计算
核心：在反向传播时不存储 P，而是通过 SRAM 中的 Q, K, V 块，重新计算一次前向的 Attention

思想 —— 计算 & 内存的权衡：多一次计算量，却能大幅减少显存带宽瓶颈和空间占用。即“用计算换 IO”
