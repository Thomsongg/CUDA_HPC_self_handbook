...
#### 2.3 使用 CUDA Stream 实现大批量数据处理
1. 使用简单的**多流并发**，实现内存数据传输  `cudaMemcpyAsync` 与 GPU 核函数调用的异步实现。
2. 将**多流并发**与**事件同步**机制结合，处理多个核函数的流间依赖问题 -> 假设 Kernel2 的输入依赖于 Kernel1 的输出
3. 掌握“流水线式”处理逻辑，将大批量的数据数据分块 (Chunks) ，使用多个流分批次处理，实现 CPU-GPU 和 GPU 的计算任务与数据传输“重叠 (Overlap)”。
